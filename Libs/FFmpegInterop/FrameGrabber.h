	...
	public:
		...
		static IAsyncOperation<FrameGrabber^>^ CreateFromStreamAsync(IRandomAccessStream^ stream)
		{
			return create_async([stream]
			{
				...
				config->PassthroughAudioMP3 = passthrough;
				config->PassthroughAudioAAC = passthrough;
				...
				config->VideoOutputAllowIyuv = passthrough;
				config->VideoOutputAllow10bit = passthrough;
				config->VideoOutputAllowBgra8 = passthrough;
				config->VideoOutputAllowNv12 = passthrough;
				config->AutoSelectForcedSubtitles = passthrough;
				config->UseEmbeddedSubtitleFonts = passthrough;
				config->VideoDecoderMode = VideoDecoderMode::ForceSystemDecoder;
