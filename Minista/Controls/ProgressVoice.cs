...
namespace Minista.Controls
{
    public class ProgressVoice : ProgressBar
    {
        ...
        private IList<float/*byte*/> _deferred;

        public void UpdateWave(List<float> waveForm)
        {
            UpdateSlide(waveForm);
        }

        private void UpdateSlide(IList</*byte*/float> waveform)
        {
            ...
            for (int i = 0; i < result.Length; i++)
            {
                ...
                //var tx = shift / 31.0;
                result[i] = waveform[j];
                //result[i] = (((int)waveform[j] | ((int)(j + 1 < waveform.Count ? waveform[j + 1] : 0) << 8)) >> shift & 0x1F) / 31.0;
            }

            //var imageWidth = 209.0;
            //var imageHeight = 24;
            var imageWidth = 102d; // double.IsNaN(ActualWidth) ? 142 : ActualWidth;
            var imageHeight = 20;
