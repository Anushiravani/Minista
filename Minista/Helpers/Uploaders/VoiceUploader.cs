...
namespace Minista.Helpers.Uploaders
{
    public class VoiceUploader
    {
        ...
        internal string GenerateUploadId()=> DateTime.UtcNow.ToUnixTimeMiliSeconds().ToString();
        static Uri GetDIrectConfigureUri()
        {
            return new Uri("https://i.instagram.com/api/v1/direct_v2/threads/broadcast/share_voice/", UriKind.RelativeOrAbsolute);
        }
        static Uri GeFinishUri()
        {
            return new Uri("https://i.instagram.com/api/v1/media/upload_finish/", UriKind.RelativeOrAbsolute);
        }
        static Uri GetMediaUploadUri(string uploadId, int fileHashCode)
        {
        ...
        readonly List<string> WaveForm = new List<string>();
       private InstaDirectInboxItem DirectItem;
        ...
        private async Task<IResult<bool>> FinishVoiceAsync()
        {
            try
            {
                Debug.WriteLine("----------------------------------------FinishVoiceAsync----------------------------------");
