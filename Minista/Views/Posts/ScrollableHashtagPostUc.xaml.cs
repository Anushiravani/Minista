...
{
    public sealed partial class ScrollableHashtagPostUc : UserControl
    {
        ...
        public ScrollableHashtagPostUc()
        {
        ...
        public void SetData(HashtagsTopGenerator vm, int index)
        {
            SelectedIndex = index;
            HashtagsTopGenerator = vm;
            try
            {
        ...
        private async void LoadData()
        {
            try
            {
                if (HashtagsTopGenerator == null) return;
                if (!CanLoadFirstPopUp)
                {
                    ...
                    LVPosts.ItemsSource = HashtagsTopGenerator.Items;
                    await Task.Delay(500);
                    if (Scroll != null)
                        Scroll.ViewChanging += HashtagsTopGenerator.ScrollViewChanging;
                    if (SelectedIndex != -1)
                        LVPosts.ScrollIntoView(HashtagsTopGenerator.Items[SelectedIndex]);
