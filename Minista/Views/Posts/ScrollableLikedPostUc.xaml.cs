...
namespace Minista.Views.Posts
{
    public sealed partial class ScrollableLikedPostUc : UserControl
    {
        ...
        public void SetData(LikedPostViewModel vm, int index)
        {
            SelectedIndex = index;
            LikedPostVM = vm;
            try
        ...
        private async void LoadData()
        {
            try
            {
                ...
                if (!CanLoadFirstPopUp)
                {
                    ...
                    LVPosts.ItemsSource = LikedPostVM.Items;
                    await Task.Delay(500);
                    if (Scroll != null)
                        Scroll.ViewChanging += LikedPostVM.ScrollViewChanging;
                    if (SelectedIndex != -1)
                        LVPosts.ScrollIntoView(LikedPostVM.Items[SelectedIndex]);
