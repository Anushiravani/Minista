...
namespace Minista.Views.Posts
{
    public sealed partial class ScrollableSavedPostUc : UserControl
    {
        ...
        public void SetData(SavedPostsViewModel vm, int index)
        {
            SelectedIndex = index;
            SavedPostsVM = vm;
            try
        ...
        private async void LoadData()
        {
            try
            {
                ...
                if (!CanLoadFirstPopUp)
                {
                    ...
                    LVPosts.ItemsSource = SavedPostsVM.Items;
                    await Task.Delay(500);
                    if (Scroll != null)
                        Scroll.ViewChanging += SavedPostsVM.ScrollViewChanging;
                    if (SelectedIndex != -1)
                        LVPosts.ScrollIntoView(SavedPostsVM.Items[SelectedIndex]);
        ...
        private void RefreshControlRefreshRequested(Microsoft.UI.Xaml.Controls.RefreshContainer sender, Microsoft.UI.Xaml.Controls.RefreshRequestedEventArgs args)
        {
            ...
            {
                if (SavedPostsVM != null)
                    SavedPostsVM.RunLoadMore(true);
            }
        }
