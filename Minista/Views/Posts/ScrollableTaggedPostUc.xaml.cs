...
namespace Minista.Views.Posts
{
    public sealed partial class ScrollableTaggedPostUc : UserControl
    {
        ...
        public void SetData(TaggedYouViewModel vm, int index)
        {
            SelectedIndex = index;
            TaggedPostVM = vm;
            try
        ...
        private async void LoadData()
        {
            try
            {
                ...
                if (!CanLoadFirstPopUp)
                {
                    ...
                    LVPosts.ItemsSource = TaggedPostVM.Items;
                    await Task.Delay(500);
                    if (Scroll != null)
                        Scroll.ViewChanging += TaggedPostVM.ScrollViewChanging;
                    if (SelectedIndex != -1)
