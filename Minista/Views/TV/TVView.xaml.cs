...
namespace Minista.Views.TV
{
    ...
    /// </summary>
    public sealed partial class TVView : Page
    {
        ...
        public static TVView Current;
        public TVView()
        {
            this.InitializeComponent();

            Current = this;
            //DataContext = ExploreClusterVM;
            ...
            Loaded += ExploreClusterViewLoaded;
        }
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            try
            {
                //if (e.Parameter != null && e.Parameter is InstaTopicalExploreCluster clsuter)
                //    Cluster = clsuter;
            }
            catch { }
        }
        ...
        private void RunSearch()
        {
            try
            {
                if (TVSearchView != null)
                    TVSearchView.SearchVM.FirstLoad();
            }
            catch { }
        }
        ...
        private /*async*/ void LVItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                e.ClickedItem.GetType().PrintDebug();
                if (e.ClickedItem is InstaMedia media)
                {
                    var index = TVVM.Items.IndexOf(media);
                    Helpers.NavigationService.Navigate(typeof(TVPlayer), new object[] { TVVM.Items.ToList(), index });
                    //var index = ExploreClusterVM.Items.IndexOf(media);
        ...
        {
            if (TVSearchView.Visibility == Visibility.Collapsed)
                TVSearchView.Visibility = Visibility.Visible;
            else
                TVSearchView.Visibility = Visibility.Collapsed;
        }
    }
}
